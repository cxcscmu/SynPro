...
class WarmupStableDecay(Scheduler):
    ...
    def get_lr(self, initial_lr: float, step: int, max_steps: int) -> float:
        ...
        decay_steps = warmup  # warmup == decay steps
        hold = max_steps - warmup - decay_steps
        hold = max(hold, 0)  # safety fallback
        eta_min = initial_lr * self.alpha_f
        if step < warmup:
            return self._linear_warmup(initial_lr, step, warmup)
        elif step < warmup + hold:
            return initial_lr
        elif step < warmup + hold + decay_steps:
            t = (step - warmup - hold) / decay_steps
            cosine_factor = 0.5 * (1 + cos(pi * t))
            return eta_min + (initial_lr - eta_min) * cosine_factor
        else:
            return eta_min
...
def get_param_groups(cfg: TrainConfig, model: nn.Module) -> List[Dict[str, Any]]:
        ...
        for pn, p in m.named_parameters():
            ...
            if pn.endswith("bias"):
                ...
                if cfg.optimizer.decay_norm_and_bias:
                    ...
                    no_decay.add(fpn)
            elif pn.endswith("weight") and "norm" in m.__class__.__name__.lower():
                # Handle custom norm modules (e.g., Qwen3RMSNorm) that are not LayerNormBase.
                if cfg.optimizer.decay_norm_and_bias:
                    decay.add(fpn)
                else:
                    no_decay.add(fpn)
    ...
    # MoE expert parameters (w1, v1, w2) and router weights are not captured above.
    # Add any uncategorized parameters to the decay set by default.
    uncategorized = all_params.keys() - (decay | no_decay)
    if uncategorized:
        decay.update(uncategorized)
...
def build_scheduler(cfg: TrainConfig, sched_cfg: Optional[SchedulerConfig] = None) -> Scheduler:
    ...
    elif sched_cfg.name == SchedulerType.warmup_stable_decay:
