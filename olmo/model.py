...
class OLMoBlock(nn.Module):
    ...
    def build(cls, layer_id: int, config: ModelConfig, cache: BufferCache) -> OLMoBlock:
        if config.block_type == BlockType.sequential:
            ...
            return OLMoMoEBlock(layer_id, config, cache)
        else:
...
class MoELayer(nn.Module):
    """Pure PyTorch Mixture-of-Experts layer. No external dependencies."""
    ...
    def __init__(self, config: ModelConfig):
        ...
        hidden_size = config.mlp_hidden_size if config.mlp_hidden_size is not None else int(config.mlp_ratio * config.d_model)

        # Router
        self.router = nn.Linear(config.d_model, self.num_experts, bias=False, device=config.init_device)

        # Expert FFNs (parameter tensors for all experts)
        if config.activation_type == ActivationType.swiglu:
            # SwiGLU: gate + up projection
            self.w1 = nn.Parameter(torch.empty(self.num_experts, hidden_size, config.d_model, device=config.init_device))
            self.v1 = nn.Parameter(torch.empty(self.num_experts, hidden_size, config.d_model, device=config.init_device))
            self.w2 = nn.Parameter(torch.empty(self.num_experts, config.d_model, hidden_size, device=config.init_device))
        ...
        self._z_loss = torch.tensor(0.0)

    def reset_parameters(self, std: float = 0.02, cutoff_factor: Optional[float] = None):
        nn.init.trunc_normal_(self.router.weight, std=std, a=-std * (cutoff_factor or 3), b=std * (cutoff_factor or 3))
        nn.init.trunc_normal_(self.w1, std=std, a=-std * (cutoff_factor or 3), b=std * (cutoff_factor or 3))
        nn.init.trunc_normal_(self.w2, std=std, a=-std * (cutoff_factor or 3), b=std * (cutoff_factor or 3))
        if self.v1 is not None:
            nn.init.trunc_normal_(self.v1, std=std, a=-std * (cutoff_factor or 3), b=std * (cutoff_factor or 3))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        orig_shape = x.shape
        x_flat = x.reshape(-1, self.d_model)  # (B*T, D)
        num_tokens = x_flat.shape[0]

        # Router: compute logits and top-k
        router_logits = self.router(x_flat)  # (B*T, num_experts)

        # Z-loss (router logit regularization)
        self._z_loss = torch.logsumexp(router_logits, dim=-1).square().mean()

        router_probs = F.softmax(router_logits, dim=-1)
        top_k_probs, top_k_indices = torch.topk(router_probs, self.top_k, dim=-1)  # (B*T, top_k)
        ...
        tokens_per_expert.scatter_add_(0, top_k_indices.reshape(-1), torch.ones(num_tokens * self.top_k, device=x.device, dtype=x.dtype))
        tokens_per_expert = tokens_per_expert / (num_tokens * self.top_k)
        mean_prob = router_probs.mean(dim=0)
        self._aux_loss = (tokens_per_expert * mean_prob).sum() * self.num_experts

        # Batched expert computation
        # Flatten top-k: treat each (token, expert_slot) as an independent token
        flat_indices = top_k_indices.reshape(-1)  # (B*T*top_k,)
        flat_weights = top_k_weights.reshape(-1)  # (B*T*top_k,)
        # Repeat input tokens for each top-k slot
        x_repeat = x_flat.unsqueeze(1).expand(-1, self.top_k, -1).reshape(-1, self.d_model)  # (B*T*top_k, D)

        # Group tokens by expert using sort
        sorted_indices = torch.argsort(flat_indices, stable=True)
        sorted_expert_ids = flat_indices[sorted_indices]
        sorted_tokens = x_repeat[sorted_indices]  # (B*T*top_k, D)
        sorted_weights = flat_weights[sorted_indices]  # (B*T*top_k,)
        ...
        # Process each expert as a batch (one GEMM per expert, not per token)
        sorted_output = torch.zeros_like(sorted_tokens)
        offset = 0
        for e in range(self.num_experts):
            cnt = expert_counts[e].item()
            if cnt == 0:
                continue
            tokens_e = sorted_tokens[offset:offset + cnt]  # (cnt, D)
            if self._use_swiglu:
                gate = F.silu(tokens_e @ self.w1[e].t())
                up = tokens_e @ self.v1[e].t()
                expert_out = (gate * up) @ self.w2[e].t()
            else:
                expert_out = F.relu(tokens_e @ self.w1[e].t()) @ self.w2[e].t()
            sorted_output[offset:offset + cnt] = sorted_weights[offset:offset + cnt].unsqueeze(-1) * expert_out
            offset += cnt

        # Unsort and aggregate
        output_flat = torch.zeros_like(x_repeat)
        output_flat[sorted_indices] = sorted_output
        # Reshape back to (B*T, top_k, D) and sum over top_k
        output = output_flat.reshape(num_tokens, self.top_k, self.d_model).sum(dim=1)

        return output.reshape(orig_shape)


class OLMoMoEBlock(OLMoBlock):
    """
    Mixture-of-Experts transformer block. Identical to OLMoSequentialBlock for
    attention, but replaces the dense FFN with a MoE layer (pure PyTorch).
    """

    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        super().__init__(layer_id, config, cache)

        # Remove unused dense FFN params from base class
        del self.ff_out
        del self.act

        # Attention input projection (same as OLMoSequentialBlock)
        head_dim = config.d_model // config.n_heads
        self.fused_dims = (
            config.d_model,
            config.effective_n_kv_heads * head_dim,
    ...
    def reset_parameters(self):
        # Don't call super().reset_parameters() — it accesses self.ff_out which we deleted.
        # Instead, reset only what we have.
        self.attn_norm.reset_parameters()
        self.ff_norm.reset_parameters()
        ...
        self.ffn.reset_parameters(std, cutoff_factor)
    ...
    def forward(
        ...
        # ---- Attention (identical to OLMoSequentialBlock) ----
        if not self.config.norm_after:
            h = self.attn_norm(x)
        else:
            h = x
        ...
        att, cache = self.attention(
            ...
            max_doc_len=max_doc_len, cu_doc_lens=cu_doc_lens,
        )
