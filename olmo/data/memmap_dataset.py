...
class MemMapDataset(Dataset[Dict[str, Any]]):
    ...
    :param path_subsampling_rates: Optional per-path sampling rates in ``(0, 1]`` used to reduce the number of
        chunks loaded from each path. The effective number of chunks for path ``i`` is
        ``max(1, floor(num_chunks_i * rate_i))`` when ``num_chunks_i > 0``.
    :param path_subsampling_seed: Base seed used for per-path random chunk subsampling when
        ``path_subsampling_rates`` is set.
    """
    ...
    def __init__(
        ...
        *paths: PathOrStr,
        ...
        if path_subsampling_rates is not None and len(path_subsampling_rates) != len(paths):
            raise ValueError("'path_subsampling_rates' should have the same length as the number of file paths")
        if path_subsampling_rates is not None:
            for i, rate in enumerate(path_subsampling_rates):
                if rate <= 0 or rate > 1:
                    raise ValueError(f"'path_subsampling_rates[{i}]' must be in the interval (0, 1], got {rate}")
        ...
        self._path_subsampling_rates = path_subsampling_rates
        self._path_subsampling_seed = path_subsampling_seed
        self._path_sampled_indices: Optional[List[Optional[np.ndarray]]] = None
    ...
    def offsets(self) -> List[Tuple[int, int]]:
        ...
        if self._mmap_offsets is None:
            ...
            for i, path in enumerate(self._memmap_paths):
                ...
                if self._path_subsampling_rates is not None:
                    rate = self._path_subsampling_rates[i]
                    sampled_indices: Optional[np.ndarray] = None
                    if length > 0:
                        sampled_length = max(1, int(length * rate))
                        if sampled_length < length:
                            rng = np.random.default_rng(self._path_subsampling_seed + i)
                            sampled_indices = np.sort(rng.choice(length, size=sampled_length, replace=False))
                        length = sampled_length
                    else:
                        length = 0
                    self._path_sampled_indices.append(sampled_indices)
                else:
                    self._path_sampled_indices.append(None)
                if mask_path_to_length:
                    mask_path = path_to_mask_path[path]
                    if length != mask_path_to_length[mask_path]:
                        if self._path_subsampling_rates is None:
                            raise ValueError(f"masking file '{mask_path}' should be the same size as '{path}'")
                        if length > mask_path_to_length[mask_path]:
                            raise ValueError(
                                f"subsampled length for '{path}' exceeds masking file '{mask_path}' size"
                            )
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self._path_sampled_indices is not None:
            sampled_indices = self._path_sampled_indices[memmap_index]
            if sampled_indices is not None:
                memmap_local_index = int(sampled_indices[memmap_local_index])
