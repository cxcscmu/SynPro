...
def build_memmap_dataset(
    ...
    if data_config.paths:
        ...
        path_subsampling_rates = data_config.path_subsampling_rates
        if path_subsampling_rates is not None and len(path_subsampling_rates) != len(paths):
            raise OLMoConfigurationError(
                "DataConfig.path_subsampling_rates must have the same length as DataConfig.paths"
            )
        if path_subsampling_rates is not None:
            for i, rate in enumerate(path_subsampling_rates):
                if rate <= 0 or rate > 1:
                    raise OLMoConfigurationError(
                        f"DataConfig.path_subsampling_rates[{i}] must be in the interval (0, 1], got {rate}"
                    )
        for path in paths:
            metadata.append({"path": str(path)})
    elif data_config.datasets:
        if data_config.path_subsampling_rates is not None:
            raise OLMoConfigurationError("DataConfig.path_subsampling_rates is only supported with DataConfig.paths")
        paths = []
    ...
    return MemMapDataset(
        *paths,
        ...
        path_subsampling_rates=path_subsampling_rates,
        path_subsampling_seed=(
            data_config.path_subsampling_seed if data_config.path_subsampling_seed is not None else train_config.seed
        ),
    )
