...
class BlockType(StrEnum):
    ...
    moe = "moe"
    """
    A Mixture-of-Experts block that replaces the FFN with a router + multiple expert FFNs.
    Requires the ``megablocks`` library.
    """
...
class ModelConfig(BaseConfig):
    ...
    model_type: str = "olmo"
    """
    The model architecture type. Use "olmo" for OLMo models or "qwen3" for Qwen3 models.
    """
    ...
    """Number of experts in MoE blocks."""

    moe_top_k: int = 2
    """Number of experts selected per token."""

    moe_dropless: bool = True
    """Use dropless MoE (dMoE) from megablocks. If False, use standard MoE with capacity factor."""

    moe_loss_weight: float = 0.1
    """Weight for the load-balancing auxiliary loss."""

    moe_zloss_weight: float = 0.0
    """Weight for the router z-loss. 0 = disabled."""

    moe_capacity_factor: float = 1.25
    """Capacity factor for standard (non-dropless) MoE."""

    moe_mlp_impl: str = "sparse"
    """MoE MLP implementation: 'sparse' or 'grouped'."""

    moe_shared_expert: bool = False
    """If True, add a shared expert that always processes all tokens (DeepSeek-style)."""

    moe_lbl_in_fp32: bool = False
    """If True, compute load-balancing loss in FP32."""

    moe_log_expert_assignment: bool = False
    """If True, log expert assignment statistics."""
...
class DataConfig(BaseConfig):
    ...
    path_subsampling_rates: Optional[List[float]] = None
    path_subsampling_seed: Optional[int] = None
...
class TrainConfig(BaseConfig):
    ...
    ```bash
    --load_path='${path.last_checkpoint:s3://your-bucket/checkpoints/7b/example-run}'
    ```
