...
class ICLMetric(Metric):
    ...
    def __init__(self, metric_type="acc") -> None:
        """metric_type: f1, acc, acc_all, len_norm, pmi_dc, ce_loss, bpb"""
    ...
    def update(self, batch: Dict[str, Any], lm_logits: torch.Tensor, dc_lm_logits=None):
        ...
        for idx, (doc_id, cont_id) in enumerate(zip(batch["doc_id"], batch["cont_id"])):
            ...
            elif self.metric_type == "acc_all":
                # 1.0 if every greedy-decoded token in the continuation matches, 0.0 otherwise
                # matches lm_eval's is_greedy semantics for single-answer tasks like LAMBADA
                greedy_tokens = lm_cont_logits.argmax(dim=-1)
                log_likelihood = torch.tensor(
                    1.0 if (greedy_tokens == cont_tokens).all() else 0.0,
                    device=lm_cont_logits.device,
                )
    ...
    def compute(self) -> torch.Tensor:
        ...
        for doc_id in loglikelihood_dict:
            ...
            if skip_document:
                continue
            if self.metric_type in ["ce_loss", "bpb", "acc_all"]:
...
class LambadaOpenAI(ICLMultiChoiceTaskDataset):
    """LAMBADA OpenAI dataset evaluated via CE loss (perplexity) on the last word.

    Each example has a single field "text". The context is everything except the last
    whitespace-delimited word; the continuation is " " + last_word.

    {
        'text': 'She could feel the butterflies in her stomach as she walked into the room. It was crowded.'
    }
    """
...
class LambadaOpenAIAcc(LambadaOpenAI):
    """LAMBADA OpenAI dataset evaluated via greedy-decode accuracy on the last word.

    Accuracy = 1 iff every token of the last word is greedily decoded correctly,
    matching the lm_eval is_greedy semantics.
    """
...
label_to_task_map = {
    ...
    "lambada_openai_acc": LambadaOpenAIAcc,
