...
class Trainer:
    ...
    def train_micro_batch(
        ...
        # MoE auxiliary losses (collected from MoELayer modules)
        if self.cfg.model.block_type == BlockType.moe:
            from .model import MoELayer
            for module in self.dist_model.modules():
                if isinstance(module, MoELayer):
                    if self.cfg.model.moe_loss_weight > 0:
                        loss = loss + self.cfg.model.moe_loss_weight * module._aux_loss
                    if self.cfg.model.moe_zloss_weight > 0:
                        loss = loss + self.cfg.model.moe_zloss_weight * module._z_loss
